			...
			Section: for (Section section : row.getSections()) {
				...
				for (Customer customer : customers) {
					if (customer.getTicketReq() == seatsAvailable) {
						customer.setSeatInfo(String.join(",", row.toString(), section.toString()));
				...
				//double customer single section
					...
					while (left < right) {
						...
						if (move == 0) {
							...
							customer1.setSeatInfo(String.join(",", row.toString(), section.toString()));
							customer2.setSeatInfo(String.join(",", row.toString(), section.toString()));
